...
import { CloseIcon } from "@/shared/components/form/editor/tiptap-icons/close-icon";
import "@/shared/components/form/editor/tiptap-node/image-upload-node/image-upload-node.scss";
import { Button } from "@/shared/components/form/editor/tiptap-ui-primitive/button";
import { appConfig } from "@/shared/configs/app.config";
import axios from "@/shared/lib/axios";
import { focusNextNode, isValidPosition } from "@/shared/lib/tiptap-utils";
...
function useFileUpload(options: UploadOptions) {
  ...
  const uploadFile = async (file: File): Promise<string | null> => {
    ...
    try {
      ...
      const { data: heroImageRes } = await axios.post(
        `/upload/image`,
        formData,
      );

      if (heroImageRes.statusCode !== 200) {
        throw new Error("Image upload failed");
      }

      const url = appConfig.PUBLIC_API_URL + heroImageRes?.data?.url;
